...
import type { Product, StockLevel, StockMovement, WarehouseRef } from "@/lib/types";
...
export default function InventoryPage() {
  ...
  const movements = useQuery({
    ...
    queryFn: () => api<StockMovement[]>(`/v1/inventory/movements?limit=50`, { orgId }),
    enabled: !!orgId,
  });
    ...
    <div className="space-y-4">
        ...
        <Card>
          ...
          {stock.isLoading ? (
            ...
            <Table headers={["Producto", "SKU", "Almacén", "Cantidad"]}>
              {stock.data?.map((row) => (
                <tr key={row.id}>
                  <td className="px-5 py-3 font-medium">{row.variant?.name ?? row.variant_id}</td>
                  <td className="px-5 py-3 text-slate-600 dark:text-slate-400">{row.variant?.sku}</td>
                  <td className="px-5 py-3">{row.warehouse?.name ?? row.warehouse_id}</td>
      ...
      <Card>
        ...
        {movements.isLoading ? (
          ...
          <Table headers={["Fecha", "Producto", "Almacén", "Tipo", "Cantidad", "Saldo", "Motivo"]}>
            {movements.data?.map((mov) => (
              <tr key={mov.id}>
                <td className="px-5 py-3 text-slate-600 dark:text-slate-400">{new Date(mov.created_at).toLocaleString()}</td>
                <td className="px-5 py-3 font-medium">{mov.variant?.name ?? mov.variant_id}</td>
                <td className="px-5 py-3">{mov.warehouse?.name}</td>
                <td className="px-5 py-3">
                  <span className={Number(mov.qty) > 0 ? "text-green-600" : "text-red-600"}>
                    {mov.movement_type}
                  </span>
                </td>
                <td className="px-5 py-3 font-semibold">{formatQty(mov.qty)}</td>
                <td className="px-5 py-3">{formatQty(mov.balance_after)}</td>
                <td className="px-5 py-3 text-slate-600 dark:text-slate-400">{mov.reason}</td>
              </tr>
            ))}
          </Table>
        )}
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      await api(`/v1/inventory/adjustments`, {
        method: "POST",
        orgId,
        body: { variant_id: variantId, warehouse_id: warehouseId, delta: Number(delta), reason: reason || null },
      });
