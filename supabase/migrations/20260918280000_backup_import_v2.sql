-- Importación con remapeo de IDs (nuevos UUIDs) preservando las relaciones.
-- Esto permite restaurar el respaldo en la misma base o en una nueva sin colisiones.
...
begin
  ...
  drop table if exists _m_branches, _m_warehouses, _m_price_lists, _m_categories, _m_brands,
    _m_products, _m_variants, _m_parties, _m_invoices, _m_pos, _m_po_lines, _m_gr, _m_si, _m_roles;
  ...
  create temp table _m_branches (old_id uuid primary key, new_id uuid not null);
  insert into _m_branches select (x->>'id')::uuid, gen_random_uuid() from jsonb_array_elements(p_data -> 'branches') x;
  insert into public.branches (id, org_id, name, code, address, is_active, created_at, updated_at)
  select b.new_id, p_org_id, x->>'name', x->>'code', x->>'address',
  ...
  join _m_branches b on b.old_id = (x->>'id')::uuid;
  v_count := v_count + (select count(*) from _m_branches);

  create temp table _m_warehouses (old_id uuid primary key, new_id uuid not null);
  insert into _m_warehouses select (x->>'id')::uuid, gen_random_uuid() from jsonb_array_elements(p_data -> 'warehouses') x;
  insert into public.warehouses (id, org_id, branch_id, name, code, is_active, created_at, updated_at)
  select w.new_id, p_org_id, b.new_id, x->>'name', x->>'code',
         coalesce((x->>'is_active')::boolean, true), coalesce((x->>'created_at')::timestamptz, now()), coalesce((x->>'updated_at')::timestamptz, now())
  from jsonb_array_elements(p_data -> 'warehouses') x
  join _m_warehouses w on w.old_id = (x->>'id')::uuid
  join _m_branches b on b.old_id = (x->>'branch_id')::uuid;

  -- ============ LISTAS / CATEGORÍAS / MARCAS ============
  create temp table _m_price_lists (old_id uuid primary key, new_id uuid not null);
  insert into _m_price_lists select (x->>'id')::uuid, gen_random_uuid() from jsonb_array_elements(p_data -> 'price_lists') x;
  insert into public.price_lists (id, org_id, code, name, is_active, created_at, updated_at)
  select pl.new_id, p_org_id, x->>'code', x->>'name',
         coalesce((x->>'is_active')::boolean, true), coalesce((x->>'created_at')::timestamptz, now()), coalesce((x->>'updated_at')::timestamptz, now())
  from jsonb_array_elements(p_data -> 'price_lists') x
  join _m_price_lists pl on pl.old_id = (x->>'id')::uuid;

  create temp table _m_categories (old_id uuid primary key, new_id uuid not null);
  insert into _m_categories select (x->>'id')::uuid, gen_random_uuid() from jsonb_array_elements(p_data -> 'categories') x;
  insert into public.categories (id, org_id, name, parent_id, is_active, created_at, updated_at)
  select c.new_id, p_org_id, x->>'name', p.new_id, coalesce((x->>'is_active')::boolean, true),
         coalesce((x->>'created_at')::timestamptz, now()), coalesce((x->>'updated_at')::timestamptz, now())
  from jsonb_array_elements(p_data -> 'categories') x
  join _m_categories c on c.old_id = (x->>'id')::uuid
  left join _m_categories p on p.old_id = nullif(x->>'parent_id','')::uuid;

  create temp table _m_brands (old_id uuid primary key, new_id uuid not null);
  insert into _m_brands select (x->>'id')::uuid, gen_random_uuid() from jsonb_array_elements(p_data -> 'brands') x;
  insert into public.brands (id, org_id, name, is_active, created_at, updated_at)
  select b.new_id, p_org_id, x->>'name', coalesce((x->>'is_active')::boolean, true),
         coalesce((x->>'created_at')::timestamptz, now()), coalesce((x->>'updated_at')::timestamptz, now())
  from jsonb_array_elements(p_data -> 'brands') x
  join _m_brands b on b.old_id = (x->>'id')::uuid;

  -- ============ PRODUCTOS ============
  create temp table _m_products (old_id uuid primary key, new_id uuid not null);
  insert into _m_products select (x->>'id')::uuid, gen_random_uuid() from jsonb_array_elements(p_data -> 'products') x;
  ...
  select p.new_id, p_org_id, c.new_id, br.new_id, x->>'name', x->>'description', coalesce(x->>'base_unit','unit'),
  ...
  join _m_products p on p.old_id = (x->>'id')::uuid
  left join _m_categories c on c.old_id = nullif(x->>'category_id','')::uuid
  left join _m_brands br on br.old_id = nullif(x->>'brand_id','')::uuid;
  v_count := v_count + (select count(*) from _m_products);

  create temp table _m_variants (old_id uuid primary key, new_id uuid not null);
  insert into _m_variants select (x->>'id')::uuid, gen_random_uuid() from jsonb_array_elements(p_data -> 'product_variants') x;
  insert into public.product_variants (id, org_id, product_id, sku, name, attributes, is_active, created_at, updated_at)
  select v.new_id, p_org_id, p.new_id, x->>'sku', x->>'name', coalesce(x->'attributes', '{}'::jsonb),
         coalesce((x->>'is_active')::boolean, true), coalesce((x->>'created_at')::timestamptz, now()), coalesce((x->>'updated_at')::timestamptz, now())
  from jsonb_array_elements(p_data -> 'product_variants') x
  join _m_variants v on v.old_id = (x->>'id')::uuid
  join _m_products p on p.old_id = (x->>'product_id')::uuid;

  insert into public.barcodes (id, org_id, variant_id, barcode, format, is_primary, created_at)
  select gen_random_uuid(), p_org_id, v.new_id, x->>'barcode', coalesce(x->>'format','EAN13'),
         coalesce((x->>'is_primary')::boolean, false), coalesce((x->>'created_at')::timestamptz, now())
  from jsonb_array_elements(p_data -> 'barcodes') x
  join _m_variants v on v.old_id = (x->>'variant_id')::uuid;

  insert into public.variant_prices (id, org_id, variant_id, price_list_id, currency, amount, created_at, updated_at)
  select gen_random_uuid(), p_org_id, v.new_id, pl.new_id, x->>'currency', (x->>'amount')::numeric,
         coalesce((x->>'created_at')::timestamptz, now()), coalesce((x->>'updated_at')::timestamptz, now())
  from jsonb_array_elements(p_data -> 'variant_prices') x
  join _m_variants v on v.old_id = (x->>'variant_id')::uuid
  join _m_price_lists pl on pl.old_id = (x->>'price_list_id')::uuid;

  -- ============ TERCEROS ============
  create temp table _m_parties (old_id uuid primary key, new_id uuid not null);
  insert into _m_parties select (x->>'id')::uuid, gen_random_uuid() from jsonb_array_elements(p_data -> 'parties') x;
  ...
  select pt.new_id, p_org_id, x->>'name', coalesce(x->>'document_type','other'), x->>'document_id',
  ...
  join _m_parties pt on pt.old_id = (x->>'id')::uuid;
  v_count := v_count + (select count(*) from _m_parties);

  -- ============ STOCK ============
  insert into public.stock_levels (id, org_id, variant_id, warehouse_id, qty, created_at, updated_at)
  select gen_random_uuid(), p_org_id, v.new_id, w.new_id, (x->>'qty')::numeric,
         coalesce((x->>'created_at')::timestamptz, now()), coalesce((x->>'updated_at')::timestamptz, now())
  from jsonb_array_elements(p_data -> 'stock_levels') x
  join _m_variants v on v.old_id = (x->>'variant_id')::uuid
  join _m_warehouses w on w.old_id = (x->>'warehouse_id')::uuid;
  ...
  select gen_random_uuid(), p_org_id, v.new_id, w.new_id, x->>'movement_type', (x->>'qty')::numeric,
         (x->>'balance_after')::numeric, nullif(x->>'reference_type',''), nullif(x->>'reference_id','')::uuid,
         x->>'reason',
         coalesce((select u.id from auth.users u where u.id = (x->>'created_by')::uuid), auth.uid()),
  ...
  -- ============ FACTURAS / PAGOS / CXC ============
  create temp table _m_invoices (old_id uuid primary key, new_id uuid not null);
  insert into _m_invoices select (x->>'id')::uuid, gen_random_uuid() from jsonb_array_elements(p_data -> 'invoices') x;
  ...
  select i.new_id, p_org_id, x->>'number', pt.new_id, (x->>'subtotal')::numeric, (x->>'tax')::numeric,
         (x->>'total')::numeric, x->>'currency', (x->>'exchange_rate')::numeric, (x->>'tax_rate')::numeric,
         coalesce(x->>'status','posted'),
         coalesce((select u.id from auth.users u where u.id = (x->>'created_by')::uuid), auth.uid()),
         coalesce((x->>'created_at')::timestamptz, now()), coalesce((x->>'updated_at')::timestamptz, now())
  from jsonb_array_elements(p_data -> 'invoices') x
  join _m_invoices i on i.old_id = (x->>'id')::uuid
  left join _m_parties pt on pt.old_id = nullif(x->>'party_id','')::uuid;
  v_count := v_count + (select count(*) from _m_invoices);

  insert into public.invoice_lines (id, org_id, invoice_id, variant_id, qty, unit_price, line_total, tax, currency, created_at)
  select gen_random_uuid(), p_org_id, i.new_id, v.new_id, (x->>'qty')::numeric, (x->>'unit_price')::numeric,
         (x->>'line_total')::numeric, (x->>'tax')::numeric, x->>'currency', coalesce((x->>'created_at')::timestamptz, now())
  from jsonb_array_elements(p_data -> 'invoice_lines') x
  join _m_invoices i on i.old_id = (x->>'invoice_id')::uuid
  join _m_variants v on v.old_id = (x->>'variant_id')::uuid;

  insert into public.payments (id, org_id, invoice_id, amount, currency, exchange_rate, method, created_by, created_at)
  select gen_random_uuid(), p_org_id, i.new_id, (x->>'amount')::numeric, x->>'currency',
         (x->>'exchange_rate')::numeric, coalesce(x->>'method','cash'),
         coalesce((select u.id from auth.users u where u.id = (x->>'created_by')::uuid), auth.uid()),
         coalesce((x->>'created_at')::timestamptz, now())
  ...
  select gen_random_uuid(), p_org_id, pt.new_id, i.new_id, x->>'entry_type', (x->>'amount')::numeric, x->>'currency',
         coalesce((select u.id from auth.users u where u.id = (x->>'created_by')::uuid), auth.uid()),
         coalesce((x->>'created_at')::timestamptz, now())
  from jsonb_array_elements(p_data -> 'ar_ledger') x
  join _m_parties pt on pt.old_id = (x->>'party_id')::uuid
  join _m_invoices i on i.old_id = (x->>'invoice_id')::uuid;

  -- ============ COMPRAS / CXP ============
  create temp table _m_pos (old_id uuid primary key, new_id uuid not null);
  insert into _m_pos select (x->>'id')::uuid, gen_random_uuid() from jsonb_array_elements(p_data -> 'purchase_orders') x;
  ...
  select po.new_id, p_org_id, pt.new_id, w.new_id, x->>'number', coalesce(x->>'status','ordered'), x->>'currency',
         (x->>'exchange_rate')::numeric, (x->>'tax_rate')::numeric, nullif(x->>'expected_at','')::date, x->>'notes',
         coalesce((select u.id from auth.users u where u.id = (x->>'created_by')::uuid), auth.uid()),
         coalesce((x->>'created_at')::timestamptz, now()), coalesce((x->>'updated_at')::timestamptz, now())
  from jsonb_array_elements(p_data -> 'purchase_orders') x
  join _m_pos po on po.old_id = (x->>'id')::uuid
  join _m_parties pt on pt.old_id = (x->>'supplier_id')::uuid
  join _m_warehouses w on w.old_id = (x->>'warehouse_id')::uuid;

  create temp table _m_po_lines (old_id uuid primary key, new_id uuid not null);
  insert into _m_po_lines select (x->>'id')::uuid, gen_random_uuid() from jsonb_array_elements(p_data -> 'po_lines') x;
  insert into public.po_lines (id, org_id, po_id, variant_id, qty_ordered, qty_received, unit_cost, line_total, currency, created_at)
  select l.new_id, p_org_id, po.new_id, v.new_id, (x->>'qty_ordered')::numeric, (x->>'qty_received')::numeric,
         (x->>'unit_cost')::numeric, (x->>'line_total')::numeric, x->>'currency', coalesce((x->>'created_at')::timestamptz, now())
  from jsonb_array_elements(p_data -> 'po_lines') x
  join _m_po_lines l on l.old_id = (x->>'id')::uuid
  join _m_pos po on po.old_id = (x->>'po_id')::uuid
  join _m_variants v on v.old_id = (x->>'variant_id')::uuid;

  create temp table _m_gr (old_id uuid primary key, new_id uuid not null);
  insert into _m_gr select (x->>'id')::uuid, gen_random_uuid() from jsonb_array_elements(p_data -> 'goods_receipts') x;
  insert into public.goods_receipts (id, org_id, po_id, number, received_at, notes, created_by, created_at)
  select g.new_id, p_org_id, po.new_id, x->>'number', coalesce((x->>'received_at')::date, current_date), x->>'notes',
         coalesce((select u.id from auth.users u where u.id = (x->>'created_by')::uuid), auth.uid()),
         coalesce((x->>'created_at')::timestamptz, now())
  from jsonb_array_elements(p_data -> 'goods_receipts') x
  join _m_gr g on g.old_id = (x->>'id')::uuid
  join _m_pos po on po.old_id = (x->>'po_id')::uuid;

  insert into public.receipt_lines (id, org_id, receipt_id, po_line_id, variant_id, qty, unit_cost, line_total, currency, created_at)
  select gen_random_uuid(), p_org_id, g.new_id, l.new_id, v.new_id, (x->>'qty')::numeric,
         (x->>'unit_cost')::numeric, (x->>'line_total')::numeric, x->>'currency', coalesce((x->>'created_at')::timestamptz, now())
  from jsonb_array_elements(p_data -> 'receipt_lines') x
  join _m_gr g on g.old_id = (x->>'receipt_id')::uuid
  join _m_po_lines l on l.old_id = (x->>'po_line_id')::uuid
  join _m_variants v on v.old_id = (x->>'variant_id')::uuid;

  create temp table _m_si (old_id uuid primary key, new_id uuid not null);
  insert into _m_si select (x->>'id')::uuid, gen_random_uuid() from jsonb_array_elements(p_data -> 'supplier_invoices') x;
  ...
  select s.new_id, p_org_id, pt.new_id, po.new_id, x->>'number', (x->>'subtotal')::numeric, (x->>'tax')::numeric,
         (x->>'total')::numeric, x->>'currency', (x->>'exchange_rate')::numeric, (x->>'tax_rate')::numeric,
         coalesce(x->>'status','posted'), nullif(x->>'due_date','')::date,
         coalesce((select u.id from auth.users u where u.id = (x->>'created_by')::uuid), auth.uid()),
         coalesce((x->>'created_at')::timestamptz, now()), coalesce((x->>'updated_at')::timestamptz, now())
  from jsonb_array_elements(p_data -> 'supplier_invoices') x
  join _m_si s on s.old_id = (x->>'id')::uuid
  join _m_parties pt on pt.old_id = (x->>'supplier_id')::uuid
  left join _m_pos po on po.old_id = nullif(x->>'po_id','')::uuid;

  insert into public.ap_ledger (id, org_id, party_id, supplier_invoice_id, entry_type, amount, currency, created_by, created_at)
  select gen_random_uuid(), p_org_id, pt.new_id, s.new_id, x->>'entry_type', (x->>'amount')::numeric, x->>'currency',
         coalesce((select u.id from auth.users u where u.id = (x->>'created_by')::uuid), auth.uid()),
         coalesce((x->>'created_at')::timestamptz, now())
  from jsonb_array_elements(p_data -> 'ap_ledger') x
  join _m_parties pt on pt.old_id = (x->>'party_id')::uuid
  join _m_si s on s.old_id = (x->>'supplier_invoice_id')::uuid;
  ...
  select gen_random_uuid(), p_org_id, v.new_id, w.new_id, (x->>'min_qty')::numeric, (x->>'max_qty')::numeric,
         (x->>'pack_multiple')::numeric, pt.new_id, (x->>'lead_time_days')::int, coalesce((x->>'is_active')::boolean, true),
         coalesce((x->>'created_at')::timestamptz, now()), coalesce((x->>'updated_at')::timestamptz, now())
  from jsonb_array_elements(p_data -> 'replenishment_config') x
  join _m_variants v on v.old_id = (x->>'variant_id')::uuid
  join _m_warehouses w on w.old_id = (x->>'warehouse_id')::uuid
  left join _m_parties pt on pt.old_id = nullif(x->>'preferred_supplier_id','')::uuid;

  insert into public.exchange_rates (id, org_id, rate_date, rate, source, created_by, created_at)
  select gen_random_uuid(), p_org_id, (x->>'rate_date')::date, (x->>'rate')::numeric, coalesce(x->>'source','manual'),
         (select u.id from auth.users u where u.id = (x->>'created_by')::uuid), coalesce((x->>'created_at')::timestamptz, now())
  ...
  -- ============ ROLES / MIEMBROS ============
  create temp table _m_roles (old_id uuid primary key, new_id uuid not null);
  insert into _m_roles select (x->>'id')::uuid, gen_random_uuid() from jsonb_array_elements(p_data -> 'org_roles') x;
  insert into public.org_roles (id, org_id, name, permissions, is_system, created_at, updated_at)
  select r.new_id, p_org_id, x->>'name',
  ...
  join _m_roles r on r.old_id = (x->>'id')::uuid;
