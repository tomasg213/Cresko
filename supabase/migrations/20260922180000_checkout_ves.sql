-- =====================================================================
-- Checkout: la factura y los pagos se expresan en Bolívares (VES).
-- Los montos de pago recibidos (p_paid_amount y p_payments[].amount)
-- se interpretan en VES y se convierten a USD internamente (ledger,
-- payments) usando la tasa de cambio del día.
-- =====================================================================
...
declare
  ...
  v_pay jsonb;
  v_pay_method text;
  v_pay_amount_ves numeric(18, 2);
  v_pay_amount_usd numeric(18, 2);
  v_pay_sum_ves numeric(18, 2) := 0;
  v_already_paid_usd numeric(18, 2) := 0;
begin
  ...
  -- Pago en Bolívares (VES). La suma de los splits define lo pagado.
  if p_payments is not null and jsonb_typeof(p_payments) = 'array' then
    ...
    loop
      v_pay_amount_ves := round(coalesce((v_pay ->> 'amount')::numeric, 0), 2);
      if v_pay_amount_ves < 0 then
        raise exception 'payment amount must not be negative';
      end if;
      v_pay_sum_ves := round(v_pay_sum_ves + v_pay_amount_ves, 2);
    end loop;
    v_paid_ves := least(v_pay_sum_ves, v_total_ves);
  else
    v_paid_ves := least(round(coalesce(p_paid_amount, 0), 2), v_total_ves);
  end if;

  v_paid_usd := round(v_paid_ves / p_exchange_rate, 2);
  v_balance_ves := greatest(round(v_total_ves - v_paid_ves, 2), 0);
  v_balance_usd := round(v_balance_ves / p_exchange_rate, 2);
  ...
  -- Registrar pagos por método (split) o un único pago, en USD internamente.
  if v_paid_usd > 0 then
    if p_payments is not null and jsonb_typeof(p_payments) = 'array' then
      ...
      loop
        ...
        v_pay_amount_ves := round(coalesce((v_pay ->> 'amount')::numeric, 0), 2);
        if v_pay_amount_ves > 0 then
          select coalesce(sum(amount), 0) into v_already_paid_usd
          from public.payments where org_id = p_org_id and invoice_id = v_invoice_id;
          v_pay_amount_usd := round(v_pay_amount_ves / p_exchange_rate, 2);
          if v_pay_amount_usd > 0 then
            insert into public.payments (
              org_id, invoice_id, amount, currency, exchange_rate, method, created_by
            )
            values (
              p_org_id, v_invoice_id,
              least(v_pay_amount_usd, v_total_usd - v_already_paid_usd),
