-- =====================================================================
-- Pedidos: productos bajo pedido.
--
-- 1. special_order_products: productos configurados como "bajo pedido"
--    (no se venden de mostrador; se encargan).
-- 2. orders: pedidos que los clientes registrados hacen sobre un
--    producto bajo pedido, con cantidad, costo y moneda. Puede pagarse
--    total o parcialmente (el saldo queda como cuenta por cobrar).
--
-- Reemplaza el modelo anterior de la migración 20260922060000_orders
-- (tablas vacías en el momento del cambio).
-- =====================================================================

-- Eliminar objetos del modelo anterior (están vacíos).
...
drop function if exists public.cresko_create_order(uuid, uuid, uuid, numeric, numeric, text, numeric, numeric, numeric, text, date, text);
...
begin
  ...
  insert into public.special_order_products (
    org_id, name, sku, description, unit_price, currency, created_by
  )
  values (
    p_org_id, trim(p_name), nullif(p_sku, ''), p_description,
    p_unit_price, p_currency, auth.uid()
  )
...
-- =====================================================================
-- Crear pedido de un cliente sobre un producto bajo pedido. El monto
-- pagado se aplica en el mismo momento; el saldo queda por cobrar.
-- =====================================================================
