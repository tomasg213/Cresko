-- =====================================================================
-- FX: tasas de cambio diarias (BCV) y facturación en VES a precios USD
-- =====================================================================
...
create index exchange_rates_org_idx on public.exchange_rates (org_id, rate_date desc);

create or replace function public.cresko_current_exchange_rate(
  p_org_id uuid,
...
as $$
  ...
  where org_id = p_org_id and rate_date <= p_rate_date
  order by rate_date desc, created_at desc
  limit 1;
$$;

create or replace function public.cresko_upsert_exchange_rate(
  p_org_id uuid,
  p_rate_date date,
  p_rate numeric,
...
begin
  ...
  if p_source not in ('bcv', 'manual') then
    raise exception 'invalid source';
  end if;

  insert into public.exchange_rates (org_id, rate_date, rate, source, created_by)
  values (p_org_id, p_rate_date, p_rate, p_source, auth.uid())
...
-- =====================================================================
-- Checkout: precios registrados en USD, factura en VES a la tasa vigente
-- =====================================================================
...
begin
  ...
  v_balance_ves := round(v_total_ves - p_paid_amount, 2);
  ...
  if p_paid_amount > 0 then
    ...
    values (
      p_org_id, v_invoice_id, round(p_paid_amount, 2), 'VES', p_exchange_rate,
...
revoke execute on function public.cresko_checkout(
  uuid, uuid, text, text, numeric, numeric, jsonb, text, numeric, uuid, jsonb
) from authenticated;

alter table public.exchange_rates enable row level security;

create policy exchange_rates_member_read on public.exchange_rates for select
  to authenticated using (public.is_org_member(org_id));
create policy exchange_rates_admin_write on public.exchange_rates for all
  to authenticated
...
grant execute on function public.cresko_current_exchange_rate(uuid, date) to authenticated;
grant execute on function public.cresko_upsert_exchange_rate(uuid, date, numeric, text) to authenticated;
